...
$expression = "ffmpeg --% -fflags nobuffer -re -f lavfi -i sine=220:4       -fflags nobuffer -re -f gdigrab -framerate 30 -i `"title=Video latency toolkit - signal generator`"               -map 0 -ac 1 -ar 32000 -c:a aac -b:a 64k       -map 1 -c:v libx264  -pix_fmt yuv420p -profile:v high -preset fast -sc_threshold 0 -x264-params keyint=15:min-keyint=15:rc-lookahead=1:bframes=0:sliced-threads=1 -b:v 1500k -minrate:v 1500k -maxrate:v 1500k -bufsize:v 150k -s 512x512             -f hls -hls_time 1 -hls_list_size 20 -method POST -hls_segment_type fmp4 -http_persistent 1 -ignore_io_errors 1  $url"
